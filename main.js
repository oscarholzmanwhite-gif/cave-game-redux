...
const textures = [
  textureLoader.load("assets/cobblestone.png"),
  textureLoader.load("assets/grass.png"),
];
...
for (let i = 0; i < worldView.length; i++) {
  ...
  if (blockData.value) {
    ...
    if (!obstructed) {
      ...
      dummy.updateMatrix();
      worldMeshes[+blockData.type0].setMatrixAt(
...
function render() {
  ...
  if (movementDir.length() > 0) {
    camera.position.add(
      movementDir.normalize().multiplyScalar(deltaTime * 4.3),
    );
  }
  if (grounded) {
    if (keyIsDown(" ")) {
  ...
  let camRounded = new THREE.Vector3(
        ...
        if (index !== false && unpackBlock(worldView[index]).value) {
          ...
          if (abX < 1 && abY < 1.5 && abZ < 1) {
            ...
            const collisions = [
              {
                axis: "x",
                overlap: overlapX,
                sign: Math.sign(displacement.x) || 1,
              },
              {
                axis: "y",
                overlap: overlapY,
                sign: Math.sign(displacement.y) || 1,
              },
              {
                axis: "z",
                overlap: overlapZ,
                sign: Math.sign(displacement.z) || 1,
              },
            ];
            const smallest = collisions.reduce((best, current) =>
              current.overlap < best.overlap ? current : best,
            );
            if (smallest.overlap > 0) {
              camera.position[smallest.axis] -=
                smallest.sign * smallest.overlap;
              velocity[smallest.axis] = 0;
            }
            let below = new THREE.Vector3(
              camRounded.x,
              camRounded.y - 1,
              camRounded.z,
            );
            ...
            if (
              ...
              displacement.y < 0 &&
              belowIndex !== false &&
              unpackBlock(worldView[belowIndex]).value
            ) {
              grounded = true;
            }
          }
  ...
  if (hits.length > 0) {
    ...
    if (event.button === 0) {
      ...
      if (worldIndex !== false) {
        ...
        if (!worldData.value) {
          const targetMesh = worldMeshes[currentType];
          dummy.position.set(targetX, targetY, targetZ);
          dummy.scale.set(1, 1, 1);
          dummy.updateMatrix();
          targetMesh.setMatrixAt(drawnCubes[currentType], dummy.matrix);
          targetMesh.instanceMatrix.needsUpdate = true;
          drawnCubes[currentType]++;
          targetMesh.count++;
          worldData.value = true;
          worldData.drawn = true;
          worldData.type0 = currentType;
        ...
        for (let value = -1; value < 2; value += 2) {
          ...
          if (worldIndex !== false) {
            ...
            if (!worldData.drawn && worldData.value) {
              ...
              dummy.updateMatrix();
              // Something's up here
              worldMeshes[+worldData.type0].setMatrixAt(
                drawnCubes[+worldData.type0],
                dummy.matrix,
              );
